...
s32 execute_mario_action(UNUSED struct Object *obj) {
    ...
    if (gMarioState->action) {
        ...
        if (gMarioState->wall != NULL) {
            if (gMarioState->wall->type == SURFACE_RED_BAR) {
                move_mario_to_respawn(gMarioState,DEATH_TYPE_BURNED);
            }
        }
        if (gMarioState->floor != NULL) {
            if ((gMarioState->floor->type == SURFACE_RED_BAR) && !(gMarioState->action & ACT_FLAG_AIR) && !(gMarioState->action & ACT_FLAG_SWIMMING) && (gMarioState->action != ACT_DEBUG_FREE_MOVE) && (gMarioState->action != ACT_STAR_DANCE_WATER)) {
                move_mario_to_respawn(gMarioState,DEATH_TYPE_BURNED);
            }
        }
